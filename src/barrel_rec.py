...
class BarrelRec(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        queries_r = queries_r.view(*queries_r.shape[:2], self.num_attention_heads, self.d_keys)  # [batch_size, seq_len, num_heads, d_keys]
        queries_w = queries_w.view(*queries_w.shape[:2], self.num_attention_heads, self.d_keys)  # [batch_size, seq_len, num_heads, d_keys]

        keys = self.keys.unsqueeze(1).expand(-1, seq_len, -1)  # [num_lines, seq_len, d_keys]
        seq_idx = torch.arange(seq_len, device=keys.device).unsqueeze(0).expand(self.num_lines, -1)
        line_idx = (torch.arange(self.num_lines, device=keys.device).unsqueeze(-1) + seq_idx) % self.num_lines
        keys = keys[line_idx, seq_idx]  # [num_lines, seq_idx, d_keys]
        
        values = values.view(*values.shape[:2], self.num_attention_heads, self.d_values)  # [batch_size, seq_len, num_heads, d_values]

        score_r = torch.einsum("blhd,mld->blhm", queries_r, keys) / (self.d_keys ** 0.5)
        score_w = torch.einsum("blhd,mld->blhm", queries_w, keys) / (self.d_keys ** 0.5)
